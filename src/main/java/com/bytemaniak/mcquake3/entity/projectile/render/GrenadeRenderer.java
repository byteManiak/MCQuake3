...
public class GrenadeRenderer extends GeoEntityRenderer<Grenade> {
    ...
    public GrenadeRenderer(EntityRendererFactory.Context ctx) {
        super(ctx, new DefaultedEntityGeoModel<>(Identifier.of("mcquake3:grenade"), true));
    }

    @Override
    public void actuallyRender(MatrixStack poseStack, Grenade animatable, BakedGeoModel model, @Nullable RenderLayer renderType, VertexConsumerProvider bufferSource, @Nullable VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
