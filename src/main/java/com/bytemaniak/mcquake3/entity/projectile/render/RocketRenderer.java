...
public class RocketRenderer extends GeoEntityRenderer<Rocket> {
    public RocketRenderer(EntityRendererFactory.Context ctx) {
        super(ctx, new DefaultedEntityGeoModel<>(Identifier.of("mcquake3:rocket"), true));
    }

    @Override
    public void actuallyRender(MatrixStack poseStack, Rocket animatable, BakedGeoModel model, @Nullable RenderLayer renderType, VertexConsumerProvider bufferSource, @Nullable VertexConsumer buffer, boolean isReRender, float partialTick, int packedLight, int packedOverlay, int colour) {
